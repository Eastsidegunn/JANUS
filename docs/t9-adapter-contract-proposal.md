...
상태: **승인됨** (2026-08-18 3차 리뷰 — SCP-002 명세 변경, 스키마·codegen,
C안 우선 실행, smoke 필수 조건 유지, API key fallback 미승인). 문서만 — contracts 변경도, 신규
...
2. ~~A키안 대비 API key 사전 승인~~ — **2026-08-18 리뷰에서 미승인.**
   C안이 실패하면 **정지하고 재제안**한다(API key를 임의로 쓰지 않는다).
3. 둘 다 불가하면 §2.2 B안 또는 다른 격리 방식으로 **재제안**.

**확정(2026-08-18)**: C안 우선 실행 승인, API key fallback 미승인.
C안 실패 시 T9는 **정지 후 재제안**이며 구현 완료로 머지할 수 없다.
...
| CI | 미포함 (네트워크·인증 필요) |
| 절차 | ① 격리 확인: pristine 작업공간에서 사용자 훅이 발화하지 않음 ② 우리 훅 발화 → `approval_request` 방출 ③ **deny 응답 시 툴 미실행** ④ **allow 응답 시 실행** ⑤ **managed policy/hook 존재 여부 기록**(`--setting-sources` 통제 범위 밖일 수 있음 — 있으면 격리 가정이 깨지므로 재제안 판단 근거) ⑥ 사용 커맨드·결과를 PR에 기록 |
