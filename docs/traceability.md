...
| FR-SBX-06 잔여 — production surface가 `world_backend:"none"`을 거부 | 미구현. 제안서 §8 6단계(surface wiring)에서 수행 — world backend가 없는 현재 `hx run` 경로를 깨뜨리므로 1단계에서 적용하지 않았다 | T10-6 | 미착수 |
| FR-SBX-05 (부분) — fs·subprocess·sandbox를 단일 `world.Backend`/`world.Lease` lifecycle로 소유, 병합 완료 정책의 불변 스냅샷 전달 | `core/world/world_test.go` (TestEffectivePolicyAndSpawnSpecAreSnapshots, TestFakeLeaseLifecycleOrderEffectsAndErrors) | T10-2 | green |
| FR-SBX-05 잔여 — local Podman 구현과 향후 remote backend가 소비자 API 변경 없이 같은 계약을 만족 | Linux `world-integration` 및 surface 조립은 제안서 §8 3~7단계에서 수행 | T10-3~7 | 미착수 |
| FR-ADP-10 (부분) — adapter broker endpoint와 overlay upper를 host-only Lease 경계로 분리 | `core/world/world_test.go` (TestUpperDirExistsOnlyOnHostLeaseBoundary, TestFakeLeaseLifecycleOrderEffectsAndErrors) | T10-2 | green |
| FR-ADP-10 잔여 — 컨테이너 agent에서 host adapter·broker/approval socket 접근 및 변조 불가 | Linux `world-integration`의 process/path/socket 비접근·adapter hash 불변 검증은 제안서 §8 5·7단계에서 수행 | T10-5,7 | 미착수 |
