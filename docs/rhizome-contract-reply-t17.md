...
## 2. 계약 개정 제안 — **v1.1로 비준 완료** (2026-09-11, [H])

> 아래 8건은 Rhizome 리뷰(T17 계약 정합 리뷰, 판정: 통과)에서 전부
> 수용·비준됐다. 잠정이 아니라 확정이다. 추가 결정: v1의 상태 조회는
> 동일 요청 재제출(무spawn 멱등 조회)로 갈음하고, 독립 Lookup 표면은
> 계약 v2로 예약한다.
...
## 4. 리뷰 결과 반영 (2026-09-11)

Rhizome 리뷰어의 T17 계약 정합 리뷰 **판정: 통과**. 독립 검증: durable
순서 코드 실측, 4대 수용 테스트 독립 실행, Linux run 34582462462 확인.

**레지스트리 불변식 예외 — [H] 조건부 수용.** 비준 조건:

1. 로그 밖 원본 기록의 예외는 accept 레지스트리 **하나로 한정**한다.
2. 레지스트리는 불변이다(수정·삭제 없음).
3. `hx audit` 또는 동급 표면이 레지스트리↔세션 로그 대조(claim·마커 ↔
   binding·session/start 일치 검증)를 지원해야 한다 — **T18 완료 전까지**
   후속 구현(TASKS.md T17-1로 등록).
4. 이 예외를 선례로 다른 로그 밖 상태를 추가하려면 별도 [H] 승인 필요.

§7 잔여 처분: 확장 미배선·replay `--from` 부재는 범위 밖 수용(--from은
필요 시 Rhizome이 계약 개정으로 요청), FR-SBX-04 상속은 기존 문서화된
부분 충족 유지. PR #75는 [H] 병합 대상이며, 병합 후 T18 착수가 승인됐다.
