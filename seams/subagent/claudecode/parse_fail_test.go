...
func TestFailClosedCases(t *testing.T) {
	cases := []struct {
		...
		{"미지 native type", []string{`{"type":"telemetry"}`}, "미지의 네이티브 이벤트"},
		{"미지 system subtype", []string{`{"type":"system","subtype":"mystery"}`}, "미지의 system subtype"},
		{"미지 assistant block", []string{initLine,
			`{"type":"assistant","message":{"content":[{"type":"image"}]}}`}, "미지의 assistant content block"},
		{"plugin_install 격리 위반", []string{`{"type":"system","subtype":"plugin_install"}`}, "격리 계약 위반"},
		{"hook_started 격리 위반", []string{`{"type":"hook_started"}`}, "격리 계약 위반"},
