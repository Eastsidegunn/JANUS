...
// TestProxyCredentialIsHostOnlyAndRedacted fixes the T20 guarantee that a
// proxy-held credential value never serializes and never renders in a string,
// while the (non-secret) name is generalized so a second vendor is a config
// change rather than a code change.
func TestProxyCredentialIsHostOnlyAndRedacted(t *testing.T) {
	const value = "sk-proxy-credential-never-log"
	credential, err := world.NewProxyCredential("CLAUDE_CODE_OAUTH_TOKEN", value, 1234567890)
	if err != nil {
		t.Fatal(err)
	}
	if credential.Value() != value || credential.Name() != "CLAUDE_CODE_OAUTH_TOKEN" {
		t.Fatal("proxy credential accessor가 값을 보존하지 않음")
	}
	for _, rendered := range []string{credential.String(), credential.GoString()} {
		if rendered != "<redacted>" || strings.Contains(rendered, value) {
			t.Fatalf("proxy credential redaction 실패: %q", rendered)
		}
	}
	...
	if strings.Contains(string(encoded), value) || string(encoded) != "{}" {
		t.Fatalf("proxy credential이 JSON에 노출됨: %s", encoded)
	}
	// A different vendor name is accepted (generalized), unlike the closed
	// SecretCapability env name.
	if _, err := world.NewProxyCredential("VENDOR2_API_KEY", value, 1); err != nil {
		t.Fatalf("일반 벤더 credential 이름을 거부함: %v", err)
	}
	// Nonsense names and empty values still fail closed.
	if _, err := world.NewProxyCredential("bad name", value, 1); err == nil {
		t.Fatal("형식에 맞지 않는 credential 이름을 수락함")
	}
	if _, err := world.NewProxyCredential("CLAUDE_CODE_OAUTH_TOKEN", "", 1); err == nil {
		t.Fatal("빈 credential 값을 수락함")
	}
}

// TestProxyInjectionValidateRejectsSeparatorsAndBadNames ensures rule fields
// stay safe for the pipe-encoded argv and carry only well-formed names.
func TestProxyInjectionValidateRejectsSeparatorsAndBadNames(t *testing.T) {
	good := world.ProxyInjection{Domain: "api.anthropic.com", Header: "Authorization", ValuePrefix: "Bearer ", CredentialName: "CLAUDE_CODE_OAUTH_TOKEN"}
	if err := good.Validate(); err != nil {
		t.Fatalf("유효 규칙을 거부함: %v", err)
	}
	bad := []world.ProxyInjection{
		{Domain: "a|b", Header: "Authorization", CredentialName: "TOKEN"},
		{Domain: "api.anthropic.com", Header: "Auth|z", CredentialName: "TOKEN"},
		{Domain: "api.anthropic.com", Header: "Bad Header", CredentialName: "TOKEN"},
		{Domain: "api.anthropic.com", Header: "Authorization", ValuePrefix: "x|y", CredentialName: "TOKEN"},
		{Domain: "api.anthropic.com", Header: "Authorization", CredentialName: "lower"},
		{Domain: "", Header: "Authorization", CredentialName: "TOKEN"},
	}
	for i, rule := range bad {
		if err := rule.Validate(); err == nil {
			t.Fatalf("bad rule[%d]가 통과함: %+v", i, rule)
		}
	}
}
