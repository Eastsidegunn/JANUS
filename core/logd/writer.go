...
func (w *Writer) Submit(ctx context.Context, rec gen.EventRecord) (int64, error) {
	...
	// admission 이후에는 반드시 ack — 루프는 Close 전까지 살아 있고
	// Close는 수락분을 전부 드레인한다.
	select {
	case r := <-sub.ack:
		return r.seq, r.err
	case <-w.drained:
...
func (w *Writer) loop(nextSeq int64) {
	...
	process := func(sub submission) {
		...
		rec := sub.rec
		rec.Seq = nextSeq
		// 기록 전 redaction 패스 (FR-LOG-08)
		if err := w.redactor.RedactEvent(&rec); err != nil {
			sub.ack <- ackResult{err: fmt.Errorf("logd: 이벤트 거부 (redaction): %w", err)}
			return
		}
		// 저장 직전 contracts 스키마 검증 — 위반 이벤트는 로그에 들어갈 수
		// 없다(T1 계약의 런타임 강제). 검증 실패는 해당 이벤트만 거부한다.
		encoded, err := json.Marshal(rec)
		if err != nil {
			sub.ack <- ackResult{err: fmt.Errorf("logd: 이벤트 거부 (직렬화 불가): %w", err)}
			return
		}
		if err := w.validators.ValidateRecord(encoded); err != nil {
			sub.ack <- ackResult{err: fmt.Errorf("logd: 이벤트 거부 (contracts 위반): %w", err)}
			return
		}
		// 커밋은 제출자의 ctx와 무관하게 완주한다 — 수락된 이벤트에 대해
		// 커밋 여부가 모호한 결과를 만들지 않는다.
		if err := w.store.Append(context.Background(), rec); err != nil {
			// store 커밋 실패는 회복 불능으로 간주 — writer 종료 상태.
			// 이후의 모든 제출은 커밋 없이 종료 오류로 ack된다.
